...
@app.get("/all") # api 01  von mir
def todo_list_template(request:Request, db:Session = Depends(get_db)):
    todos = db.query(ToDoModel).all()  # return all todos from db : select & from ToDo
    return template.TemplateResponse("todos.html", {"request":request, "todos":todos})

# fastapi div app.py --port 8080 ; # ps fastapi, kill id
# uvicorn app:app --reload ; # uvicorn test02:app --reload --port 9001
# Get-Process uvicorn; # Stop-Process -Id <ProcessId>
